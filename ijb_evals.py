...
import sys 
sys.path.append("/remote/idiap.svm/temp.biometric01/prahimi/exps/proj/arcface_torch")
from backbones import get_model as arc_get_model
sys.path.append("/remote/idiap.svm/temp.biometric01/prahimi/exps/proj")
# Populate from settings 
from dvpf import get_model, kwargs


class Torch_model_interf:
    def __init__(self, model_file, dvpf_kwargs, image_size=(112, 112)):
        import torch
        ...
        if 'r50' in model_file:
            print("######\nR50"*5) 
            self.arc ='r50'
        elif 'r100' in model_file:
            self.arc = 'r100'

        self.enable_dvpf = False
        if len(dvpf_kwargs.keys()) != 0:
            import json 
            print(f"Instantiating dvpf model: {json.dumps(dvpf_kwargs, indent=4, ensure_ascii=False)}")
            self.dvpf_model = get_model(dvpf_kwargs)
            #self.dvpf_model.to(self.device)
            self.enable_dvpf = True

        self.model = arc_get_model(self.arc, fp16=True)
...
class ONNX_model_interf:
    def __init__(self, model_file, dvpf_kwargs=kwargs, image_size=(112, 112)):
        import onnxruntime as ort
        ort.set_default_logger_severity(3)
        self.ort_session = ort.InferenceSession(model_file, providers=["CUDAExecutionProvider"])
    ...
    def __call__(self, imgs):
        ...
        if len(dvpf_kwargs.keys()) != 0:
            outputs = torch.from_numpy(outputs[0]).to("cuda")
            outputs = self.dvpf_model(outputs).detach().cpu()
            outputs = [outputs]
...
class IJB_test:
    def __init__(self, model_file, data_path, subset, batch_size=64, force_reload=False, restore_embs=None, dvpf_kwargs=dict()):
        ...
        if model_file != None:
            if model_file.endswith(".h5"):
                ...
                interf_func = Torch_model_interf(model_file, dvpf_kwargs)
            elif model_file.endswith(".onnx") or model_file.endswith(".ONNX"):
                print("Modified Parsa ... ")
                interf_func = ONNX_model_interf(model_file, dvpf_kwargs)
            else:
...
if __name__ == "__main__":
    ...
    if args.plot_only != None and len(args.plot_only) != 0:
        ...
        import json
        if args.dvpf_enable: 
            with open(args.dvpf_kwargs_path) as file:
                dvpf_kwargs = json.load(file)
        else: 
            dvpf_kwargs = dict()
        
        tt = IJB_test(args.model_file, args.data_path, args.subset, args.batch_size, args.force_reload, args.save_result, dvpf_kwargs= dvpf_kwargs if args.dvpf_enable else dict())
        ...
        if args.dvpf_enable:
            ...
            save_path = os.path.join(save_path, datetime.datetime.now().astimezone().replace(microsecond=0).isoformat())
        ...
        if args.is_one_2_N:
            ...
            fig.savefig(os.path.join(save_path, f"cmc.{fig_format}"))
        else:
            from pandas import DataFrame
            tpr_result_df: DataFrame = None
            tpr_result_df, fig = plot_roc_and_calculate_tpr(scores, names=names, label=label)
            fig.savefig(os.path.join(save_path, f"roc.{fig_format}"))
            from pickle import dump
            tpr_file = open(os.path.join(save_path, f"tpr_result.csv"), 'w')
            tpr_result_df.to_csv(tpr_file)